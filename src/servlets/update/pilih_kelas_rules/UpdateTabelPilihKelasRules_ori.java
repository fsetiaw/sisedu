	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isu==null) {
			...
			if(submit.equalsIgnoreCase("update")) {
				...
				if(target_kdpst!=null && target_kdpst.length>0) {
					...
					//for(int i=0;i<target_kdpst.length;i++) {
					for(int i=0;i<1;i++) {
						//System.out.println("target_kdpst["+i+"]="+target_kdpst[i]);
						String all_shift = ""+request.getParameter("all_shift_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(all_shift)) {
							all_shift = "null";
						}
						String tkn_shift = ""+request.getParameter("tkn_shift_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(tkn_shift)) {
							tkn_shift = "null";
						}
						String list_npm_shift = ""+request.getParameter("list_npm_shift_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(list_npm_shift)) {
							list_npm_shift = "null";
						}
						String all_prodi = ""+request.getParameter("all_prodi_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(all_prodi)) {
							all_prodi = "null";
						}
						String tkn_prodi = ""+request.getParameter("tkn_prodi_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(tkn_prodi)) {
							tkn_prodi = "null";
						}
						String list_npm_prodi = ""+request.getParameter("list_npm_prodi_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(list_npm_prodi)) {
							list_npm_prodi = "null";
						}
						
						String all_fak = ""+request.getParameter("all_fak_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(all_fak)) {
							all_fak = "null";
						}
						String tkn_fak = ""+request.getParameter("tkn_fak_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(tkn_fak)) {
							tkn_fak = "null";
						}
						String list_npm_fak = ""+request.getParameter("list_npm_fak_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(list_npm_fak)) {
							list_npm_fak = "null";
						}
						
						String all_kmp = ""+request.getParameter("all_kmp_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(all_kmp)) {
							all_kmp = "null";
						}
						String tkn_kmp = ""+request.getParameter("tkn_kmp_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(tkn_kmp)) {
							tkn_kmp = "null";
						}
						String list_npm_kmp = ""+request.getParameter("list_npm_kmp_"+target_kdpst[i]);
						if(Checker.isStringNullOrEmpty(list_npm_kmp)) {
							list_npm_kmp = "null";
						}
						//String list_objid_kdpst =  request.getParameter("list_objid_"+target_kdpst[i]);
						
						String[] list_objid_kdpst =  request.getParameterValues("list_objid_"+target_kdpst[i]);
						String tmp = "";
